...
const getSimilarCourses = async (req, res) => {
    try {
        ...
        .sort({ rank: 1 }) // rank generated by spark, lower is better
        .limit(5);

        const recommendedCourseIds = recommendations.map(entry => entry.recommended_course_id);

        let similar = await Course.find({ _id: { $in: recommendedCourseIds } });

        // 3. Robust Fallback: If no ML matches, provide "Safe" recommendations
        if (similar.length === 0) {
